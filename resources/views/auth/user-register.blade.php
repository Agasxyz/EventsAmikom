...
<head>
    ...
    <meta name="description" content="Buat akun AmikomEventHub dan mulai temukan serta pesan tiket event impianmu.">
    ...
    <style>
        ...
        .google-btn:hover { transform: translateY(-1px); box-shadow: 0 8px 20px rgba(0,0,0,0.15); }
    </style>
</head>
<body class="bg-indigo-900 text-white min-h-screen flex items-center justify-center p-6 py-12">
    ...
    <div class="max-w-md w-full bg-white text-slate-900 rounded-[2rem] p-8 shadow-2xl">
        ...
        <div class="text-center mb-8">
            ...
            <p class="text-slate-500 mt-1">AmikomEventHub — Temukan event impianmu</p>
        </div>
        ...
        <form action="{{ route('user.register.post') }}" method="POST" class="space-y-5">
            ...
            <div>
                ...
                <input type="text" name="name" value="{{ old('name') }}" required
                    class="w-full px-5 py-4 bg-slate-50 border-2 {{ $errors->has('name') ? 'border-rose-400' : 'border-slate-100' }} rounded-2xl focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-600 outline-none transition font-medium"
            ...
            <div>
                ...
                <input type="email" name="email" value="{{ old('email') }}" required
                    class="w-full px-5 py-4 bg-slate-50 border-2 {{ $errors->has('email') ? 'border-rose-400' : 'border-slate-100' }} rounded-2xl focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-600 outline-none transition font-medium"
            ...
            <div>
                ...
                <input type="password" name="password" required
                    class="w-full px-5 py-4 bg-slate-50 border-2 {{ $errors->has('password') ? 'border-rose-400' : 'border-slate-100' }} rounded-2xl focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-600 outline-none transition font-medium"
            ...
            <button type="submit"
                class="w-full py-4 bg-indigo-600 text-white rounded-2xl font-black text-xl shadow-lg shadow-indigo-200 hover:bg-indigo-700 transition mt-2">
                Buat Akun
            </button>
        </form>
        ...
        <div class="text-center mt-6 space-y-3">
            <div>
                <span class="text-sm text-slate-400 font-medium">Sudah punya akun?</span>
                <a href="{{ route('user.login') }}" class="text-sm text-indigo-600 hover:text-indigo-700 font-bold ml-1 transition">
                    Masuk di sini
                </a>
            </div>
            <div>
