    ...
    <form action="{{ route('organizer.events.update', $event->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        ...
        <div>
            ...
            <textarea name="description" rows="5" 
                class="w-full px-5 py-4 bg-slate-50 border-2 border-slate-100 rounded-2xl focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-600 outline-none transition font-medium resize-none">{{ old('description', $event->description) }}</textarea>
        ...
        <div>
            ...
            <div class="mt-3 flex items-center gap-2 p-3 bg-slate-50 rounded-xl border border-slate-100">
                <img src="{{ asset('storage/' . $event->poster_path) }}" class="w-12 h-16 rounded object-cover shadow-sm">
                <p class="text-xs text-slate-500">Poster saat ini terunggah. Unggah file baru untuk menggantinya.</p>
            </div>
            @endif
